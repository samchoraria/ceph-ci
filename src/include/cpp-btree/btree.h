...
public:
  ...
  void rebalance_right_to_left(int to_move, btree_node *sibling, allocator_type *alloc);
  void rebalance_left_to_right(int to_move, btree_node *sibling, allocator_type *alloc);
...
private:
 ...
 public:
  ...
  iterator insert_multi(const key_type &key, ValueType value);
...
template <typename P>
inline void btree_node<P>::remove_value(int i, allocator_type *alloc) {
  if (!leaf() && count() > i + 1) {
...
template <typename P>
void btree_node<P>::rebalance_right_to_left(int to_move,
                                            btree_node *right,
                                            allocator_type *alloc) {
...
template <typename P>
void btree_node<P>::rebalance_left_to_right(int to_move,
                                            btree_node *right,
                                            allocator_type *alloc) {
  assert(parent() == right->parent());
  assert(position() + 1 == right->position());
...
auto btree<P>::operator=(btree &&x) noexcept -> btree & {
  if (this != &x) {
    ...
    if constexpr (std::allocator_traits<
                  allocator_type>::propagate_on_container_copy_assignment::value) {
      // Note: `root_` also contains the rightmost and the size.
      swap(root_, x.root_);
