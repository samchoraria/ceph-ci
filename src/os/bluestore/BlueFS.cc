...
/*
  dout priority hierarchy
  1 - initializations in/out, any critical error
  2 - initialization results
  3 - allocation failures
  5 - init/cleanup logs
  8 - gifting/reclaiming BlueFS space
 10 - RocksDB API in/out
 15 - RocksDB API results/errors
 20 - BlueFS functions in/out
 15 - 25 BlueFS functions internals
 30 - raw dump
 */
...
int BlueFS::maybe_verify_layout(const bluefs_layout_t& layout) const
{
  if (super.memorized_layout) {
    if (layout == *super.memorized_layout) {
      dout(5) << __func__ << " bluefs layout verified positively" << dendl;
    } else {
      derr << __func__ << " memorized layout doesn't fit current one" << dendl;
      return -EIO;
    }
  } else {
    dout(5) << __func__ << " no memorized_layout in bluefs superblock"
             << dendl;
  }
...
int BlueFS::_write_super(int dev)
{
  ...
  encode(crc, bl);
  dout(5) << __func__ << " super block length(encoded): " << bl.length() << dendl;
  dout(5) << __func__ << " superblock " << super.version << dendl;
  dout(5) << __func__ << " log_fnode " << super.log_fnode << dendl;
  ceph_assert_always(bl.length() <= get_super_length());
  bl.append_zero(get_super_length() - bl.length());

  bdev[dev]->write(get_super_offset(), bl, false, WRITE_LIFE_SHORT);
  dout(5) << __func__ << " v " << super.version
           << " crc 0x" << std::hex << crc
...
{
  dout(5) << __func__ << (noop ? " NO-OP" : "") << dendl;
  ...
  dout(5) << __func__ << " log file size was 0x"
...
int BlueFS::_expand_slow_device(uint64_t need, PExtentVector& extents)
{
  ...
  if (slow_dev_expander) {
    ...
    dout(8) << __func__ << " expanding slow device by 0x"
		      ...
		      bluefs_fnode_t* node)
{
  ...
  if (!alloc[id] ||
    ...
    if (id != BDEV_SLOW) {
      if (bdev[id]) {
	dout(3) << __func__ << " failed to allocate 0x" << std::hex << len
		<< " on bdev " << (int)id
    ...
    dout(3) << __func__ << " unable to allocate 0x" << std::hex << len
