...
seastar::future<FuturizedStore::OmapIterator> CyanStore::get_omap_iterator(
    CollectionRef ch,
    const ghobject_t& oid)
{
  auto c = static_cast<Collection*>(ch.get());
  ...
  return seastar::make_ready_future<FuturizedStore::OmapIterator>(CyanStore::CyanOmapIterator(o));
}
