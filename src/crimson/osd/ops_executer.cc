...
OpsExecuter::call_errorator::future<> OpsExecuter::do_op_call(OSDOp& osd_op)
{
  ...
  try {
    ...
    return ceph::ct_error::invarg::make();
  }
  ...
  if (r) {
    ...
    if (r == -ENOENT) {
      ...
      return ceph::ct_error::permission_denied::make();
    }
    return ceph::ct_error::input_output_error::make();
  }
  ...
  return call_errorator::errorize{ seastar::async(
    ...
    (auto outcome) -> call_errorator::future<> {
      ...
      if (ret < 0) {
        return ceph::stateful_ec{ std::error_code(-ret, std::generic_category()) };
      }
      return seastar::now();
    }
  );
