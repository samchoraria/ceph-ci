...
TEST(KernelDevice, Ticket45337) {
  ...
  std::cout << bdev.path << std::endl;
  BlockDevice* b = BlockDevice::create(g_ceph_context, bdev.path, NULL, NULL,
  ...
  auto* ioc = new IOContext(g_ceph_context, NULL);

  std::cout << "opened at " << bdev.path << std::endl;
  std::cout << "writing " << bl.length() << std::endl;
  auto r = b->aio_write(0, bl, ioc, buffered);
  ASSERT_EQ(r, 0);
  ...
  ASSERT_EQ(r, 0);
  ASSERT_TRUE(memcmp(s.c_str(), outbuf, sizeof(outbuf)) == 0);
