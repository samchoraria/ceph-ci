...
public:
  ...
  TestOp *next(RadosTestContext &context) override
  {
	...
	if (m_set_redirect && !m_set_chunk) {
	  ...
	  uint32_t rand_tgt_offset = rand() % max_len;
	  while (rand_tgt_offset + rand_length > max_len) {
	    rand_tgt_offset = rand() % max_len;
	  }
	  ...
	  return new SetChunkOp(m_op, &context, oid.str(), rand_offset, rand_length, oid2.str(), 
