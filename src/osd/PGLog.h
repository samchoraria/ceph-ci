...
public:
  ...
  struct FuturizedStoreLogReader {
    ...
    void process_entry(crimson::os::FuturizedStore::OmapIterator &p) {
      if (p.key()[0] == '_')
	return;
      ceph::bufferlist bl = p.value();//Copy bufferlist before creating iterator
      auto bp = bl.cbegin();
      if (p.key() == "divergent_priors") {
	decode(divergent_priors, bp);
	ldpp_dout(dpp, 20) << "read_log_and_missing " << divergent_priors.size()
	...
	return seastar::repeat([this, iter]() mutable {
	  ...
	  process_entry(iter);
	  return iter.next().then([](int) { return seastar::stop_iteration::no; });
	});
