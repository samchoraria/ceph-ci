...
namespace s3selectEngine
{
	...
	struct csvStateMch_ : public msm::front::state_machine_def<csvStateMch_>
	{
		...
		char get_char(){return *input_cur_location;}

		char get_next_char(){input_cur_location++;return *input_cur_location;}

		const char* currentLoc(){return input_cur_location;}
		...
		void start_new_token(event_eol const&) {if(!token_idx)return; (*tokens)[ token_idx ] = start_token;token_idx++;}

		void start_new_token(event_end_of_stream const&) {} 

		void in_new_token(event_not_column_sep const&) {if(!*start_token)start_token = input_cur_location;}

		void in_new_token(event_eol const&) {*input_cur_location=0;}

		void in_new_token(event_end_of_stream const&) {}

		void in_new_token(event_column_sep const&) {(*tokens)[ token_idx ] = input_cur_location+1;*input_cur_location=0;token_idx++;}
		
		void in_new_token(event_quote const&) {if(!*start_token)start_token = input_cur_location;}
		...
		void in_quote(event_eol const&){*input_cur_location=0;}

		void in_quote(event_end_of_stream const&){*input_cur_location=0;}
		...
		void push_escape_pos() { if(escape_idx && has_esc[ escape_idx -1]== (int)(token_idx-1)) return;has_esc[ escape_idx ] = token_idx-1;escape_idx++; }
		void in_escape(event_escape const&){push_escape_pos();}
