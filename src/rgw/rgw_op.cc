    ...
    if (!store->svc()->zone->get_zonegroup().equals(s->bucket->get_info().zonegroup)) {
      ...
      if (store->svc()->zone->get_zonegroup().is_master_zonegroup() && s->system_request) {
        ...
        /* If op is get bucket location, don't redirect */
          ...
          rgw::sal::RGWObject::empty(s->object)) {
...
void RGWPutObj::execute()
{
  ...
  if (multipart) {
    ...
    processor.emplace<MultipartObjectProcessor>(
        &*aio, store, s->bucket, pdest_placement,
    ...
    processor.emplace<AppendObjectProcessor>(
            &*aio, store, s->bucket, pdest_placement, s->bucket_owner.get_id(),
	    obj_ctx, s->object->get_obj(),
            s->req_id, position, &cur_accounted_size, this, s->yield);
  } else {
    if (s->bucket->versioning_enabled()) {
  ldpp_dout(this, 20) << "DFG 14 " << op_ret << dendl;
      if (!version_id.empty()) {
  ldpp_dout(this, 20) << "DFG 15 " << op_ret << dendl;
        s->object->set_instance(version_id);
      } else {
	s->object->gen_rand_obj_instance_name();
        version_id = s->object->get_instance();
  ldpp_dout(this, 20) << "DFG 16 version_id=" << version_id << " " << op_ret << dendl;
      }
    }
    pdest_placement = &s->dest_placement;
...
void RGWPostObj::execute()
{
  ...
  do {
    ...
    rgw::sal::RGWObject* obj = s->bucket->get_object(rgw_obj_key(get_current_filename()));
    if (s->bucket->versioning_enabled()) {
    ...
    using namespace rgw::putobj;
    AtomicObjectProcessor processor(&*aio, store, s->bucket,
                                    &s->dest_placement,
                                    s->bucket_owner.get_id(),
...
void RGWDeleteObj::execute()
{
  ...
  if (!rgw::sal::RGWObject::empty(s->object)) {
    ...
    if (op_ret < 0) {
      ...
      ldout(s->cct, 20) << "TIE 2 r="<< op_ret << dendl;
      if (check_obj_lock) {
	/* check if obj exists, read orig attrs */
	if (op_ret == -ENOENT) {
	  ldout(s->cct, 20) << "TIE 3 r="<< op_ret << dendl;
  ...
  s->object->set_obj_size(s->obj_size);
  const auto ret = rgw::notify::publish(s, s->object, ceph::real_clock::now(), attrs.attrs[RGW_ATTR_ETAG].to_str(),
...
int RGWCopyObj::verify_permission()
{
  ...
  src_object->set_bucket(src_bucket);
...
void RGWGetACLs::execute()
{
  ...
  RGWAccessControlPolicy* const acl = \
    (!rgw::sal::RGWObject::empty(s->object) ? s->object_acl.get() : s->bucket_acl.get());
...
void RGWPutACLs::execute()
{
  ...
  RGWAccessControlPolicy* const existing_policy = \
    (rgw::sal::RGWObject::empty(s->object) ? s->bucket_acl.get() : s->object_acl.get());
...
#define ACL_GRANTS_MAX_NUM      100
  ...
  if ((rgw::sal::RGWObject::empty(s->object)) && !store->svc()->zone->is_meta_master()) {
    bufferlist in_data;
...
void RGWAbortMultipart::execute()
{
  ...
  if (upload_id.empty() || rgw::sal::RGWObject::empty(s->object))
    return;
                                 ...
                                 AlignedStreamGetter& body)
{
  ...
  rgw::sal::RGWObject* obj = bucket->get_object(object);
  ...
  using namespace rgw::putobj;
  AtomicObjectProcessor processor(&*aio, store, bucket, &s->dest_placement, bowner.get_id(),
