                      ...
                      req_state * const s)
{
  ...
  ldout(s->cct, 2) << "authorizing request using OPA" << dendl;

  /* get OPA url */
  const string& opa_url = s->cct->_conf->rgw_opa_url;
  if (opa_url == "") {
    ldout(s->cct, 2) << "OPA_URL not provided" << dendl;
    return -ERR_INVALID_REQUEST;
  }
  ldout(s->cct, 2) << "OPA URL= " << opa_url.c_str() << dendl;
  ...
  int ret;
  bufferlist bl;
  RGWHTTPTransceiver req(s->cct, &bl);
  ...
  /* send request */
  ret = req.process("POST", opa_url.c_str());
  if (ret < 0) {
    ldout(s->cct, 2) << "OPA process error:" << bl.c_str() << dendl;
    return ret;
  }
  ...
  if (!parser.parse(bl.c_str(), bl.length())) {
    ldout(s->cct, 2) << "OPA parse error: malformed json" << dendl;
    return -EINVAL;
  }
  ...
  if (opa_result == false) {
    ldout(s->cct, 2) << "OPA rejecting request" << dendl;
    return -EPERM;
  }

  ldout(s->cct, 2) << "OPA accepting request" << dendl;
  return 0;
}
