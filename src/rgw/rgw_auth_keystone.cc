  ...
  const boost::string_view& access_key_id) const
{
  ...
  ceph::bufferlist token_body_bl;
  RGWGetAccessSecret secret(cct, &token_body_bl);
  ...
  /* send request */
  ret = secret.process(keystone_url.c_str());
  if (ret < 0) {
			    ...
			    const signature_factory_t& signature_factory) const
{
  ...
  if (token) {
    ...
    std::tie(secret, failure_reason) = get_secret_from_keystone(
      token->get_user_id(), access_key_id);
