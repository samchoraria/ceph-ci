...
class RGWStore : public DoutPrefixProvider {
  public:
    ...
    virtual int get_bucket(RGWUser* u, const RGWBucketInfo& i, RGWBucket** bucket) = 0;
    virtual int get_bucket(RGWUser* u, const std::string& tenant, const std::string&name, RGWBucket** bucket) = 0;
    virtual RGWObject* get_object(const rgw_obj_key& k) = 0;
    virtual RGWObject* dup_object(RGWObject* obj) = 0;
...
public:
  ...
  void add(RGWBucket* bucket) {
    buckets[bucket->info.bucket.name] = bucket;
  }
...
class RGWRadosBucket : public RGWBucket {
  ...
  public:
    ...
    virtual RGWObject* get_object(const rgw_obj_key& k) override;
...
class RGWRadosStore : public RGWStore {
  ...
  public:
    ...
    virtual RGWObject* get_object(const rgw_obj_key& k) override;
    virtual RGWObject* dup_object(RGWObject* obj) override;
    virtual int get_bucket(RGWUser* u, const rgw_bucket& b, RGWBucket** bucket) override;
    virtual int get_bucket(RGWUser* u, const RGWBucketInfo& i, RGWBucket** bucket) override;
    virtual int get_bucket(RGWUser* u, const std::string& tenant, const std::string&name, RGWBucket** bucket) override;
    virtual int create_bucket(RGWUser& u, const rgw_bucket& b,
                            const string& zonegroup_id,
                            const rgw_placement_rule& placement_rule,
                            const string& swift_ver_location,
