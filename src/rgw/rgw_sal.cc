			       ...
			       uint64_t max, bool need_stats, RGWBucketList &buckets)
{
  ...
  for (const auto& ent : ulist.get_buckets()) {
    RGWRadosBucket *rb = new RGWRadosBucket(this->store, ent.second, this);
    buckets.add(rb);
  }
...
RGWObject* RGWRadosBucket::get_object(const rgw_obj_key& k)
{
  return new RGWRadosObject(this->store, k, this);
}
...
RGWObject* RGWRadosStore::get_object(const rgw_obj_key& k)
{
  return new RGWRadosObject(this, k);
}

RGWObject* RGWRadosStore::dup_object(RGWObject* _o)
{
  RGWRadosObject* obj = static_cast<RGWRadosObject *>(_o);

  return new RGWRadosObject(*obj);
}
...
int RGWRadosStore::get_bucket(RGWUser* u, const RGWBucketInfo& i, RGWBucket** bucket)
{
  RGWBucket* bp;
...
int RGWRadosStore::get_bucket(RGWUser* u, const std::string& tenant, const std::string&name, RGWBucket** bucket)
{
  rgw_bucket b;
