...
int RGWUser::execute_rename(RGWUserAdminOpState& op_state, std::string *err_msg)
{
  ...
  std::unique_ptr<rgw::sal::RGWUser> old_user(new rgw::sal::RGWRadosUser(store,
							  op_state.get_user_info()));
  std::unique_ptr<rgw::sal::RGWUser> new_user(new rgw::sal::RGWRadosUser(store,
							  op_state.get_new_uid()));
  ...
  do {
    ...
    for (it = m.begin(); it != m.end(); ++it) {
      ...
      ret = bucket->link(new_user.get(), null_yield);
      if (ret < 0) {
        set_err_msg(err_msg, "failed to link bucket " + bucket->get_name());
        return ret;
      }

      ret = bucket->chown(new_user.get(), old_user.get(), null_yield);
      if (ret < 0) {
