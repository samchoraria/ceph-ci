...
void RocksDBStore::add_column_family(const std::string& cf_name, uint32_t hash_l, uint32_t hash_h,
  ...
  bool exists = cf_handles.count(cf_name);
                                                         ...
                                                         const string &end)
{
  ...
  if (p_iter == db->cf_handles.end()) {
    ...
    for (auto cf : p_iter->second.handles) {
      ...
      for (it->Seek(start);
	   it->Valid() && db->comparator->Compare(it->key(), end) && (--cnt) != 0;
	   it->Next()) {
	bat.Delete(cf, it->key());
      }
      if (cnt == 0) {
...
public:
  ...
  // 3. go next() on all other iterators
  // 4. sort
  int prev() override {
    ...
    //4
    //insert highest in the beginning, and roll back values until we pick highest 
    ...
    //debug check
    for (size_t i = 0; i < iters.size() - 1; i++) {
      ceph_assert(keyless(iters[i], iters[i + 1]));
    }
    return 0;
  }
  bool valid() override {
