...
function check() {
    ...
    FIRST=$(grep "pg[[]${PG}.*backfilling.*_update_calc_stats degraded " $log | head -3 | tail -1 | sed "s/.* \([0-9]*\)$/\1/")
    ...
    # Check 3rd line at start because of false backfill starts
    FIRST=$(grep "pg[[]${PG}.*backfilling.*_update_calc_stats misplaced " $log | head -3 | tail -1 | sed "s/.* \([0-9]*\)$/\1/")
