...
class TestSnapSchedules(CephFSTestCase):
    ...
    def test_snap_schedule(self):
        ...
        def verify_added(snaps_added):
            ...
            if snapname.startswith('scheduled-') and snapname[10:] == snap_sfx:
                ...
                log.info("exec={}, now = {}, timo = {}".format(exec_time, now, timo))
                self.assertTrue((now - exec_time) >= timo - 1)
                return True
            return False
    ...
    def test_multi_snap_schedule(self):
        ...
        self.fs_snap_schedule_cmd("add", TestSnapSchedules.TEST_DIRECTORY, '3m')

        timo_1, snap_sfx_1 = self.calc_wait_time_and_snap_name(exec_time, '2m')
        print("expecting snap {0}/.snap/scheduled-{1} in ~{2}s...".format(TestSnapSchedules.TEST_DIRECTORY, snap_sfx_1, timo_1))
        timo_2, snap_sfx_2 = self.calc_wait_time_and_snap_name(exec_time, '3m')
        ...
        # verify snapshot schedule
        self.verify_schedule(TestSnapSchedules.TEST_DIRECTORY, [['2m', ''], ['3m', '']])
        ...
        def verify_added_1(snaps_added):
            ...
            if snapname.startswith('scheduled-') and snapname[10:] == snap_sfx_1:
                ...
                # snap should not prematurely come into existance
                self.assertTrue((now - exec_time) >= timo_1 - 1)
                return True
            return False
        def verify_added_2(snaps_added):
            ...
            if snapname.startswith('scheduled-') and snapname[10:] == snap_sfx_2:
                ...
                # snap should not prematurely come into existance
                self.assertTrue((now - exec_time) >= timo_2 - 1)
                return True
            return False
    ...
    def test_snap_schedule_with_retention(self):
        ...
        # set a schedule on the dir
        exec_time = time.time()
        self.fs_snap_schedule_cmd("add", TestSnapSchedules.TEST_DIRECTORY, '2m', 'retention-policy=3m')

        timo_1, snap_sfx = self.calc_wait_time_and_snap_name(exec_time, '2m')
