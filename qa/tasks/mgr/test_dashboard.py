...
class TestDashboard(MgrTestCase):
    ...
    def test_standby(self):
        ...
        def _is_standby():
            ...
            standbys = [standby['name'] for standby in mgr_map['standbys']]
            log.info("Current standby managers: {}".format(",".join(standbys)))
            return original_active_id in standbys

        # Force failover and wait until previous active manager is
        # listed as standby.
        self.mgr_cluster.mgr_fail(original_active_id)
        self.wait_until_true(_is_standby, timeout=30)
